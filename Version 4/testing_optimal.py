...
if __name__ == "__main__":
    ...
    for num in [100, 1000]:
        ...
        measure(run_thread_pool, num, f"Thread Pool (CPU, cores={cores})",
                cpu_task, 1000000, max_workers=cores)

        # IO-bound tasks (test multiple pool sizes)
        measure(run_new_threads, num, "New Threads (IO, sleep=0.1)", io_task, 0.1)
        for multiplier in [2, 4, 8]:
            pool_size = cores * multiplier
            measure(run_thread_pool, num,
                    f"Thread Pool (IO, cores*{multiplier}={pool_size})",
                    io_task, 0.1, max_workers=pool_size)